<?php
$output = $title = $interval = $el_class = $wd_icon = $wd_image_checkbox  = '';
...
foreach ( $tab_titles as $tab ) {
	...
	if(isset($tab_atts['title'])) {
		...
		if (isset($tab_atts['wd_image_checkbox'])) {

			if ($tab_atts['wd_image_checkbox'] == 'yes'){
	      $img_id    = preg_replace( '/[^\d]/', '', $tab_atts['wd_image'] );
   	...
   	if (isset($tab_atts['wd_icon'])) {
   		$tabs_nav .= '<i class="fa '.$tab_atts['wd_icon'] .' tab-icon"></i>';
   	}
	     }
...
if ( 'vc_tour' == $this->shortcode ) {
	$output .= "\n\t\t\t" . '<div class="wpb_tour_next_prev_nav vc_clearfix"> <span class="wpb_prev_slide"><a href="#prev" title="' . esc_attr__( 'Previous tab', 'copallyt' ) . '">' . esc_html__( 'Previous tab', 'copallyt' ) . '</a></span> <span class="wpb_next_slide"><a href="#next" title="' .esc_attr__( 'Next tab', 'copallyt' ) . '">' .esc_html__( 'Next tab', 'copallyt' ) . '</a></span></div>';
}
