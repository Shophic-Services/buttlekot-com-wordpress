...
if (!function_exists('copallyt_add_mega_menu_support')):

    function copallyt_add_mega_menu_support()
    {


        add_filter('wp_setup_nav_menu_item', 'copallyt_add_custom_nav_fields');

        add_action('wp_update_nav_menu_item', 'copallyt_update_custom_nav_fields', 10, 3);

        add_filter('wp_edit_nav_menu_walker', 'copallyt_edit_menu_walker', 10, 2);
...
add_action('init', 'copallyt_add_mega_menu_support');


/* Add custom fields to menu */

if (!function_exists('copallyt_add_custom_nav_fields')):

    function copallyt_add_custom_nav_fields($menu_item)
    {


        $menu_item->mega_menu_icon = get_post_meta($menu_item->ID, '_copallyt_mega_menu_icon', true) ? get_post_meta($menu_item->ID, '_copallyt_mega_menu_icon', true) : '';

        $menu_item->mega_menu_bg_image = get_post_meta($menu_item->ID, '_copallyt_mega_menu_bg_image', true) ? get_post_meta($menu_item->ID, '_copallyt_mega_menu_bg_image', true) : '';

        $menu_item->mega_menu = get_post_meta($menu_item->ID, '_copallyt_mega_menu', true) ? 1 : 0;
...
if (!function_exists('copallyt_update_custom_nav_fields')):

    function copallyt_update_custom_nav_fields($menu_id, $menu_item_db_id, $args)
    {


        $value = isset($_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id]) ? $_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id] : '';
        update_post_meta($menu_item_db_id, '_copallyt_mega_menu_icon', $value);


        $value = isset($_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id]) ? $_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id] : '';
        update_post_meta($menu_item_db_id, '_copallyt_mega_menu_bg_image', $value);


        $value = isset($_REQUEST['menu-item-mega-menu'][$menu_item_db_id]) ? 1 : 0;
        update_post_meta($menu_item_db_id, '_copallyt_mega_menu', $value);
...
if (!function_exists('copallyt_edit_menu_walker')):

    function copallyt_edit_menu_walker($walker, $menu_id)
    {


        class copallyt_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit
        {
            ...
            public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
            {
                ...
                $mega_menu_html .= '<p class="field-custom description description-wide">
		        		...
		        		<select id="edit-menu-item-mega-' . esc_attr($item->db_id) . '" name="menu-item-mega-menu-icon[' . esc_attr($item->db_id) . ']">';
                ...
                foreach ($icons as $key => $value) {
                    ...
                    $mega_menu_html .= "<option vlaue='" . esc_attr($value) . "' " . esc_attr($selected) . ">" . esc_html($value) . "</option>";

                }

                $mega_menu_html .= '</select>

		                ' . esc_html__('display icone', 'copallyt') . '</label>
                ...
                $mega_menu_html .= '<p class="field-custom description description-wide">
			        		...
			        		<input type="text" id="edit-menu-item-mega-' . esc_attr($item->db_id) . '" class="widefat code edit-menu-item-custom" name="menu-item-mega-menu-bg-image[' . esc_attr($item->db_id) . ']" value="' . esc_attr($item->mega_menu_bg_image) . '"  />
						
			                ' . esc_html__('Mega Menu (Background image)', 'copallyt') . '
                ...
                $mega_menu_html .= '<p class="field-custom description description-wide">
		                ...
		                ' . esc_html__('Mega Menu (classic)', 'copallyt') . '
        ...
        return 'copallyt_Walker_Nav_Menu_Edit';
